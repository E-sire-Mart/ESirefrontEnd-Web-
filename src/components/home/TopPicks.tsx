...
interface Product {
	...
	image: string; // public URL like /assets/...
}
...
const ProductCard: React.FC<{ product: Product }> = ({ product }) => (
	<div className="bg-white rounded-lg p-3 shadow-sm flex flex-col justify-between">
		...
		<div className="space-y-1">
			...
			<button className="w-full py-1.5 text-xs sm:text-sm text-blue-600 border border-blue-600 rounded hover:text-white hover:bg-blue-600 transition-colors">
				ADD TO CART
			</button>
		</div>
	</div>
...
const TopPicks: React.FC<TopPicksProps> = ({ bannerDatas, topPicksDatas }) => {
	const [currentSlide, setCurrentSlide] = useState<number>(0);
	const [slides, setSlides] = useState<Array<{ banner: Banner; products: Product[] }>>([]);
	...
	useEffect(() => {
		if (topPicksDatas?.length && bannerDatas?.length) {
			...
			const newSlides: Array<{ banner: Banner; products: Product[] }> = [];
	...
	return (
		<div className="first-carousel mx-auto px-2 sm:px-4 my-4 sm:my-8">
			...
			<div className="rounded-xl sm:rounded-2xl p-3 sm:p-4 bg-gradient-to-br from-slate-900 via-indigo-900 to-violet-800 shadow-xl">
						...
						<div className="relative w-full h-full">
							...
							{slides.length > 1 && (
								<div className="flex justify-between w-full absolute top-1/2 -translate-y-1/2 px-4">
									<button onClick={prevSlide} className="bg-white p-2 rounded-full shadow-lg z-10 hover:bg-white/90 transition-colors">
										<ChevronLeftIcon className="h-6 w-6 text-black" />
									</button>
									<button onClick={nextSlide} className="bg-white p-2 rounded-full shadow-lg z-10 hover:bg-white/90 transition-colors">
